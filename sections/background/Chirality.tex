...
\end{align}
For simplicity, we can rewrite equation~\ref{eq:background:chirality:tellegensRelations} in terms of $\varepsilon = \varepsilon_0 \varepsilon_r$, $\mu = \mu_0 \mu_r$, and $\gamma = \kappa \sqrt{\mu_0 \varepsilon_0}$ to give:
\begin{align}
...
\end{equation}
By equating expressions for $\mathbf{D}$ obtained from equations~\ref{eq:background:chirality:maxwellH} and~\ref{eq:background:chirality:tellegensSimplifiedD} we obtain:
...
\end{equation}
where $k_0=\omega / c_0$ is the wave number in vacuum (and $c_0=1/\sqrt {{\mu _0}{\varepsilon _0}}$ is the speed of light in vacuum). The eigenwaves of wave numbers ${{k}_{+}}$ and ${{k}_{-}}$ correspond to right and left circularly polarized light respectively.
From this, the corresponding refractive indices of the medium for RCP and LCP light are given by equation~\ref{eq:background:chirality:npmKappa}~\cite{Capolino2009, Qiu2008, Valev2013b}. 

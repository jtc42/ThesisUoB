...
\begin{equation}
    ...
    d\theta d\phi d\psi
\end{equation}
Similar to chapter~\ref{sec:results:OAinPlanarNanohelices}, $\beta_{ijk}^{(2)} (\theta, \phi, \psi)$ describes an effective hyper-polarisability tensor after rotation about the $z$, $y$, and $x$ axis by angles $\theta$, $\phi$, and $\psi$ respectively. In full tensor notation this is described in terms of the roation matrices (given in section~\ref{sec:background:NonlinearOptics:rotation}) by
\begin{equation}
    \begin{split}
        \beta_{ijk}^{(2) \prime} (\theta, \phi, \psi) =
